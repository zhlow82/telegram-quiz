<template>
  ...
  <div v-if="step === 1" key="step1" class="max-w-2xl space-y-5">
    ...
    <div class="bg-green-50 border border-green-200 rounded-xl px-4 py-3 flex items-start gap-2.5 text-sm text-green-800">
      <CheckCircle class="w-4 h-4 flex-shrink-0 mt-0.5" />
      <span>Already have a bot? You can <strong class="font-semibold">reuse an existing bot token</strong> — skip the steps above and just paste your token in the next step.</span>
    </div>
  </div>
